...
<script>
    ...
    $(document).ready(function() {
        ...
        @if(isset($draftData))
          // Populate form fields
            ...
            $('#surat_undangan').val(''); // File input cannot be pre-filled
            $('#ditugaskan_sebagai').val('{{ $draftData['ditugaskan_sebagai'] ?? '' }}');
            $('#alamat_kegiatan').val('{{ $draftData['alamat_kegiatan'] ?? '' }}');
            $('#provinsi').val('{{ $draftData['provinsi'] ?? '' }}');
            $('#nomor_surat_usulan').val('{{ $draftData['nomor_surat_usulan'] ?? '' }}');
            $('#pembiayaan_value').val('{{ $draftData['pembiayaan'] ?? 'Polban' }}');
            $('input[name="pembiayaan_option"][value="{{ $draftData['pembiayaan'] ?? 'Polban' }}"]').prop('checked', true);
            $('#pagu_desentralisasi_checkbox').prop('checked', {{ $draftData['pagu_desentralisasi'] ? 'true' : 'false' }});
            $('#pagu_nominal').val('{{ $draftData['pagu_nominal'] ?? '' }}');
            $('#pagu_nominal_input_group').toggle($('#pagu_desentralisasi_checkbox').is(':checked'));

            // Reinitialize daterangepicker with draft data
            let tanggalPelaksanaan = '{{ $draftData["tanggal_pelaksanaan"] ?? "" }}';
            @if(isset($draftData) && isset($draftData['tanggal_pelaksanaan']))
                var dates = '{{ $draftData["tanggal_pelaksanaan"] }}'.split(' → ');
                if (dates.length === 2) {
                    $('#tanggal_pelaksanaan').data('daterangepicker').setStartDate(moment(dates[0].trim(), 'DD/MM/YYYY'));
                    $('#tanggal_pelaksanaan').data('daterangepicker').setEndDate(moment(dates[1].trim(), 'DD/MM/YYYY'));
                    $('#tanggal_pelaksanaan').val('{{ $draftData["tanggal_pelaksanaan"] }}');
                }
            @endif
            // Initialize selected personnel
            @if(isset($selectedPersonel) && count($selectedPersonel) > 0)
              selectedPersonel = @json($selectedPersonel);
              initializeSelectedPersonel();
              selectedPersonel.forEach(personel => {
                  $(`input.personel-checkbox[data-id="${personel.id}"][data-type="${personel.type}"]`).prop('checked', true);
              });
          @endif
        @endif
    });
    ...
    $('#save-draft').on('click', function () {
        ...
        const draftId = '{{ $draft->surat_tugas_id ?? '' }}';
        if (draftId) {
            formData.append('draft_id', draftId);
            formData.append('_method', 'PUT'); // For Laravel to recognize as update
        }

        // Determine the correct URL
        const url = draftId ? "{{ route('pengusul.update.pengusulan', ['draft_id' => ':draft_id']) }}".replace(':draft_id', draftId) 
                            : "{{ route('pengusul.store.pengusulan') }}";
        ...
        fetch(url, {
            method: draftId ? 'POST' : 'POST', // Use POST for both, with _method for PUT
            body: formData,
            headers: {
        ...
        .then(data => {
            if (data.success) {
                Swal.fire('Draft Disimpan!', data.message || 'Draft berhasil disimpan.', 'success')
                    .then(() => window.location.href = '{{ route("pengusul.draft") }}');
            } else {
    ...
    $('#submit-surat').on('click', function () {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (draftId) {
                        formData.append('draft_id', draftId);
                        formData.append('_method', 'PUT'); // Gunakan PUT untuk update draft yang sudah ada
                    }

                    // Tentukan URL yang tepat
                    const url = draftId ? "{{ route('pengusul.update.pengusulan', ['draft_id' => ':draft_id']) }}".replace(':draft_id', draftId) 
                                        : "{{ route('pengusul.store.pengusulan') }}";
                    ...
                    fetch(url, {
                        method: 'POST', // Selalu gunakan POST, _method akan handle PUT
                        body: formData,
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                            'Accept': 'application/json'
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            .then(() => window.location.href = '{{ route("pengusul.status") }}');
                        } else {
