...
$(document).ready(function() {
    ...
    // Use global variables passed from Blade
    if (window.oldPembiayaanValue) {
        $('input[name="pembiayaan_option"][value="' + window.oldPembiayaanValue + '"]').prop('checked', true);
    }
    if (window.oldPaguDesentralisasiChecked !== undefined) {
        $('#pagu_desentralisasi_checkbox').prop('checked', window.oldPaguDesentralisasiChecked);
    }

    $('#pagu_nominal_input_group').toggle($('#pagu_desentralisasi_checkbox').is(':checked'));

    // Handle validation errors from Blade
    if (window.laravelErrors && window.laravelErrors.length > 0) {
        let errorHtml = 'Mohon periksa kembali input Anda:<br><ul>';
        window.laravelErrors.forEach(error => {
...
$('#save-draft').on('click', function () {
    ...
    fetch(window.storePengusulanRoute, { // Use the global variable here
